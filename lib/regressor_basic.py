...
class Regressor(object):
    def __init__(self, epoch, batch_size, model_dir, lrFE, lrSV, lrREG, lossREG, beta1, beta2, device = "cuda:0"):
    ...
    def train(self, tr_in, tr_lb, ts_in, ts_sv):
        ...
        for epoch in range(self.epoch):
            ...
            if ((epoch + 1) % 10000) == 0:
                ...
                start_time = time.clock()
                print("Epoch: [%2d] SV_loss_train: %.8f SV_loss_test: %.8f Time: [%2d]" %
                    ((epoch + 1), np.sqrt(SV_loss.item()), np.sqrt(SV_loss_test), time_iter50))
        self.save(epoch+1)
    ...
    def ABItest(self, tr_in, tr_sv, ts_in, ts_sv):
        ...
        r2score = r2_score(abi_sv.flatten(), abi_cali.flatten())
        fig = plt.figure(figsize = (8, 5))
        ...
        plt.xlim([0, ylim * 100])
        plt.ylim([0, 90])
        plt.title('MSE: ' + str(np.round_(R_loss_abi, 3)) + ',  R2: ' + str(np.round_(r2score, 3)))
        plt.xlabel('Real Severity (Inclusion [%])', fontsize = 20)
