...
class Regressor(object):
    ...
    def train(self, tr_in, tr_lb, ts_in, ts_sv):
        ...
        for epoch in range(self.epoch):
            ...
            N_loss = self.lossREG * (PWV_loss + L_loss)
            ...
            self.REG_PWV_optimizer.zero_grad()
            PWV_loss.backward(retain_graph=True)
            self.REG_PWV_optimizer.step()
            ...
            if ((epoch + 1) % 10000) == 0:
                ...
                print("Epoch: [%2d] SV_loss_train: %.8f REG_loss_train: %.8f SV_loss_test: %.8f Time: [%2d]" %
                    ((epoch + 1), np.sqrt(SV_loss.item()), np.sqrt(PWV_loss.item()) + np.sqrt(L_loss.item()), np.sqrt(SV_loss_test), time_iter50))
        self.save(epoch+1)    
