...
class BlandAltman(object):
    ...
    def plots(self, y_real, y_pred, randint, save_path):
        fig = plt.figure(figsize = (8, 5))
        data1 = np.reshape(y_real[randint], -1) * 100
        data2 = np.reshape(y_pred[randint], -1) * 100
        my_bland_altman_plot = self.bland_altman_plot(data1, data2)
        fig.savefig(save_path)
        plt.close(fig)

    def bland_altman_plot(self, m1, m2,
                        sd_limit=1.96,
                        ax=None,
                        scatter_kwds=None,
                        mean_line_kwds=None,
